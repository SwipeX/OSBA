...
public class Node extends Container {
	...
	protected static void addNodeHooks(Container container, ClassNode cn) {
		...
		if (pattern.find(cn)) {
			...
			container.addHook("getPrevious", next.name, next.owner, cn.name, container.getDescriptor(), -1);
			container.addHook("getNext", prev.name, prev.owner, cn.name, container.getDescriptor(), -1);
		}
	}
}
