...
public class Updater implements Runnable {
    ...
    public final Container[] transforms = {new Node(this), new CacheableNode(this), new NodeDeque(this),
    ...
    public Container getContainer(Class<? extends Container> type) {
        for (Container c : transforms)
            if (c.getClass() == type)
                return c;
        return null;
    }
    ...
    public void writeProHooks(Map<String, Container> containers) throws IOException {
        ...
        for (Map.Entry<String, Container> entry : containers.entrySet()) {
            ...
            try {
                out.println(Crypto.encrypt("FAPPING_ON_SWIPE_ON_TOILET", String.format("$%s#%s", container.cn.name, impl)));
            } catch (Exception e) {
        ...
        for (Map.Entry<String, Container> entry : containers.entrySet()) {
            ...
            for (Hook hook : container.hooks) {
                ...
                try {
                    out.println(Crypto.encrypt("FAPPING_ON_SWIPE_ON_TOILET", String.format("@%s,%s,%s,%s,%s,%s,%s", hook.name, hook.field, hook.clazz, hook.desc, hook.toInject, hook.multiplier, isStatic)));
                } catch (Exception e) {
                    e.printStackTrace();  //To change body of catch statement use File | Settings | File Templates.
                }
            }
